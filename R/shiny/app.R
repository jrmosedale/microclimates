...
# For mac
root<-"/Volumes/Pocket Sam/"; in.root<-"/Volumes/Pocket Sam/wineclim/"
...
dir_about<-paste(dir_shinydata,"about/",sep="")
dir_vineyards<-paste(dir_shinydata,"vineyards/",sep="")
...
# Script for display loading image. Ref: https://github.com/daattali/advanced-shiny/blob/master/plot-spinner/app.R
mycss <- "
#map-container {
position: relative;
}
...
getmap<-function(county,var,stat_year){ 
  print(paste(dir_rasters,var,"_",stat_year,"_",county,".tif",sep=""))
...
############################################
ui <- dashboardPage(
  dashboardHeader(title="Wiine-clim"),
  
  dashboardSidebar(
    sidebarMenuOutput("menu")
  ),
  
  dashboardBody(  # tags$head(tags$style(HTML(mycss))),
    tabItems(
      ...
      tabItem( tabName="explore_summary",
              fluidRow(height=100,
                       box(width=3 ,
                           selectInput(inputId = "sum_county",label="Geographical area", 
                           ...
                           selectInput(inputId = "sum_stat",label="Summary statistic (1983-2013)", 
                                       ...
                                       selected="mean",selectize=TRUE  ) 
                       ) , # box
                       tabBox(id="sum_tabVariables", width=8 ,
                              
                              tabPanel("Growing season temperature",
                                       selectInput("sum_variable1","Select data", c("Choose one"="","Degree days (base 10C)" = "gdd10_gs","Degree days (base 5C)" = "gdd5_gs", 
                              ...
                              tabPanel("Frost Risks",  
                                       selectInput("sum_variable2","Select data", c("Choose one"="","Last spring frost" = "lastspfr_doy","First autumn frost" = "firstautfr_doy", 
                              ...
                              tabPanel("Flowering Risks",  
                                       selectInput("sum_variable3","Select data", c("Choose one"="","Mean flowering temperature" = "fl_tmean",
              ...
              fluidRow( height=200,
                        box(width=10,
                          # div( id = "map-container",tags$img(src = "spinner.gif", id = "loading-spinner")),
      ...
      tabItem(tabName="search",
              fluidRow(height=30, h4(" Define area & summary statistic"), 
                            box(width=4,
...
#  } # end search_map_text
  ...
  create_search_map_text<-function(vars,mins,maxs,searchcounty,search_stat) { 
      ...
      output$searchmaptext<-renderUI({  # Add more  information such as mean for map etc
        line0<-"Highlighted locations where:"
        line1<-paste(statlabel,var_longlabel(vars[1]),"between",mins[1],"and",maxs[1])
        if (vars[2]!="") line2<-paste(statlabel,var_longlabel(vars[2]),"between",mins[2],"and",maxs[2]) else line2<-""
        if (vars[3]!="") line3<-paste(statlabel,var_longlabel(vars[3]),"between",mins[3],"and",maxs[3]) else line3<-""
        HTML(paste(line0, line1, line2, line3, sep = '<br/>'))
      })
  } # end search_map_text
  
  show_cellstats<-function(lat,lon,var){
  ...
  output$sum_maptext<-renderText({
    req(input$sum_county)
    req(input$sum_stat)
    req(sum_chosenvar())
    paste("Map showing ",input$sum_stat," of ", var_longlabel(sum_chosenvar()),"for",input$sum_county, sep=" ")
  })
  
  output$sum_mapmessage<-renderText({
    #paste("Click on cell to show values")
   output$sum_mapmessage<-renderText(paste("<font color=\" #0000FF \"><b>","Click on map for further info", "</b></font>",sep=""))
  })
  ...
  observe({  #Observer to show Popups on click
    ...
    if (!is.null(click)) {
      ...
      withProgress(message="Calculating cell values...",value=0.2,{
        lon=click$lng
        lat=click$lat
        cellnumber<-lonlat_to_cellnumber(sum_chosenlayer(),lon,lat)
        value = as.numeric(sum_chosenlayer()[cellnumber])
        content <- paste("Lon=", round(lon, 2),"; Lat=", round(lat, 2),"; Value=",round(value,0)," ",var_unit(sum_chosenvar()),sep="")
        proxy <- leafletProxy("summary_map")
        ...
        output$sum_celltext<-renderUI({  # Add more  information such as mean for map etc
          ...
          line2<-paste(var_longlabel(chosenvar())," = ",round(value,0),sep="")
          lineskip=""
          ...
          HTML(paste(line1, line2, lineskip,line3,line4,line5,line6,line7, sep = '<br/>'))
        })
      }) # withP
        # Shows histogram
        withProgress(message="Calculating frequency histogram...",value=0.5,{
        ...
        withProgress(message="Calculating timeseries plot...",value=0.8,{
  ...
  sum_chosenvar <- reactive({
    if (input$sum_tabVariables == "temp") input$sum_variable1 else 
      if (input$sum_tabVariables == "frost") input$sum_variable2 else 
        if (input$sum_tabVariables == "flower") input$sum_variable3 else 
          if (input$sum_tabVariables == "terrain") input$sum_variable4 
  })
  ...
  observe({  #Observer to show Popups on click
    ...
    if (!is.null(click)) {
      ...
      content <- paste("Lon=", round(lon, 2),"; Lat=", round(lat, 2),"; Value=",round(value,0)," ",var_unit(chosenvar()),sep="")
      proxy <- leafletProxy("map")
      proxy %>% clearPopups() %>% addPopups(lon, lat, popup = content)
      mapstats<-mapstats()
...
# Modify slider input range according to variables chosen
  ...
  observeEvent(input$startsearch,{
    ...
    leafletProxy("searchmap") %>% 
      clearPopups() %>%
      clearImages() %>% 
      addRasterImage(create_search_map(vars,mins,maxs,input$searchcounty,input$search_stat), 
                     color=colorBin(c("grey","green"),domain=c(0,1),bins=2),opacity = visibility(),project=FALSE)   
    
    create_search_map_text(vars,mins,maxs,input$searchcounty,input$search_stat)
  })
