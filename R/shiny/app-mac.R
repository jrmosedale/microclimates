  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName="wineclim",
              ...
              p(a(href="http://www.bluesky-world.com/","•	Bluesky: digital terrain map")),
      ...
      tabItem( tabName="explore_summary",
              ...
              fluidRow( height=200,
                        box( leafletOutput("summary_map"), width=10
                        ) , # box
...
#  } # end search_map_text
  ...
  output$summary_map<-renderLeaflet({
    leaflet() %>% setView(lng = -4.5, lat = 50.75, zoom = 8) %>% 
      addTiles()  %>%  
      clearImages() %>% 
      # addRasterImage(sum_chosenlayer(),color=pal, opacity = 0.7,project=FALSE) %>%
      addScaleBar()
  })
