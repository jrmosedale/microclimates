...
server<-function(input,output) {
  ...
  output$map<-renderLeaflet({
    leaflet() %>% addTiles()  %>% addRasterImage(getmap("elevation"),  color=pal, opacity = 0.7,project=FALSE)
  })
...
# NOTES - FUTURE OPPS
# add setView(lon, lat, 4) to base map start & show only basetiles
# Display values with mouse click - is it poss for multiple layers of info?? - use observeEvent(input$MAPID_click)
# Opacity slider - view/hide base tiles
# Locate county shape files - to pre-crop rasters for each area and add recalc stack
# Multiple layer display??
# Find locations UI screen - set gdd and other risk ranges to locate
# Add vineyard locations shape file???
# Re-write using layers - select if displayed rather than load new raster?

# Side Panel
# Display histograms of variable distributions for whole 'county' - locate click cell values
