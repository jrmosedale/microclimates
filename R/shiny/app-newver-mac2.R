...
server<-function(input,output,session) {
  ...
  show_popup <- function(var,value,lon=NULL,lat=NULL,mapstats) { 
    #print(value)
    ...
    output$celltext<-renderUI({  # Add more  information such as mean for map etc
      ...
      line2<-paste(var_longlabel(var)," = ",round(value,0),sep="")
      lineskip=""
      line3<-"Statistics for whole county:"
  ...
  observe({  #Observer to show Popups on click
    ...
    if (!is.null(click)) {
      ...
      show_popup(chosenvar(),value,lon,lat,mapstats())
      # Plot hist
