...
mapcolour <- function(var,mnmx) {
  mnmx<-unlist(mnmx)
  switch(var,
         "gdd10_gs" = colorBin("YlOrRd",domain=mnmx,bins=9,pretty=TRUE),
         ...
         "tmin_year" = colorBin(c("blue","white"),domain=mnmx,bins=10,pretty=TRUE),
         "t20_gsdays"= colorBin(c("white","yellow","red"),domain=mnmx,bins=10,pretty=TRUE),
         "t25_gsdays"= colorBin(c("yellow","red"),domain=mnmx,bins=10,pretty=TRUE),
         "t30_gsdays"= colorBin("OrRd",domain=mnmx,bins=10,pretty=TRUE),
         ...
         "slope"=colorBin("Purples",domain=mnmx,bins=9,pretty=TRUE),
...
get_timeseries<-function(county,var){ 
  if (var %in% c("elevation","slope","asepct")) "" else 
...
set_min_max<-function(var){
  switch(var,
         "gdd10_gs" = list(0,2000),
         "gdd5_gs" = list(0,3000),
         "tmean_gs" = list(5,30),
         "tmax_year" = list(20,50),
         "tmin_year" =list(-10,10),
         "t20_gsdays"=list(0,200),
         "t25_gsdays"=list(0,200),
              ...
              fluidRow(height=100,
                box(width=3 ,
                    ...
                    selectInput(inputId = "year",label="Time period", 
                                   choices=  c("1983" = "1983","2009" = "2009","2010" = "2010","2011" = "2011","2012" = "2012") ,
                ...
                tabBox(id="tabVariables", width=8 ,
                  ...
                  tabPanel("Frost Risks",  
                  	 ...
                  	 selectInput("variable3","Select data", c("Choose one"="","Mean flowering temperature" = "fl_numday",
                  	                                          "Good flowering days" = "fl_tmean") ,
                  	             selected="",selectize=TRUE  ),
      ...
      tabItem(tabName="search",
              fluidRow(height=30, h4("  Define area & time period"), 
                            ...
                            selectInput(inputId = "searchyear",label="Time period", 
                                        choices=  c("2009" = "2009","2010" = "2010","2011" = "2011","2012" = "2012") ,
                                        selected="2012",selectize=TRUE ) 
                            )
                        ), # fluidRow
...
server<-function(input,output,session) {
  ...
  observe({  #Observer to show Popups on click
    ...
    if (!is.null(click)) {
      ...
      output$hist<-renderPlot( plothist(chosenlayer()) + geom_vline(xintercept = value,color = "red", size=1) )
      # Plot timeseries if appropriate
      #show_timeseries(chosendata(),cellnumber)
      if (chosendata()!=""){
        load(chosendata()) # load var.m
...
# Modify slider input range according to variables chosen
  ...
  # Plot histogram
  show_hist<-function(values,value){
    ...
    output$hist<-renderPlot(ggplot(dat, aes(x = breaks, y = counts, fill =counts)) + ## Note the new aes fill here
