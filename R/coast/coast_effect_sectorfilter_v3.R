...
invdist.sector.filter<-function(m, radius, res=100, v1, v2) {  
  ...
  for (row in 1:nrow(m)){
    for (col in 1:ncol(m)){
      ...
      #ccwv1<-areClockwise(x,y,v1xy[,"x"],v1xy[,"y"])
      if ( (dist<=radius) & 
           ( areClockwise2(v2xy[,"x"],v2xy[,"y"],xmax,ymax ) & !areClockwise2(v1xy[,"x"],v1xy[,"y"],xmax,ymax ) |
             areClockwise2(v2xy[,"x"],v2xy[,"y"],xmax,ymin ) & !areClockwise2(v1xy[,"x"],v1xy[,"y"],xmax,ymin ) |
              areClockwise2(v2xy[,"x"],v2xy[,"y"],xmin,ymax ) & !areClockwise2(v1xy[,"x"],v1xy[,"y"],xmin,ymax ) |
               areClockwise2(v2xy[,"x"],v2xy[,"y"],xmin,ymin ) )  &  !areClockwise2(v1xy[,"x"],v1xy[,"y"],xmin,ymin )
            ) { m[row,col]<-dist  }     
    }
  }
  #sect.cells<-length(which(m>0))
