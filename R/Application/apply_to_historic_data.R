...
ukcpcell<-936 # for testing only lizRD - 936-939
...
#for (ukcpcell in(1:length(landcells))){
  ...
  for (jd in start.jd:end.jd ) 
  {
    ...
    for (hr in 0:23)  #loop for hours
    {
      ...
      print("Radiation")
      rad.results<-radiation_downscale(day,month,year,hr,demuk,dem.buffer,dem.block,slope.buffer,aspect.buffer,dir_rad,print.results=FALSE,write.files=FALSE )
      ...
      anom.block<-(params$estimates[1]+elev.tdif+   # difference in temperature due to altitude
               ...
               params$estimates[3]*total.block+      # net short wave radiation
      ...
      plot(aspect.block,main=paste("Aspect",timelabel,sep=""))
...
# coast.effect = invwstr.block * ldif.block + (wstr.block*sst.dif) + (ldif.block*sst.dif) # coastal effects
# but set to NA for sea and 0 if land without coast effect based on sst.dif - USES dem.land - where sea=NA, all land=0
#coast.effect<-overlay(crop(dem.land,dem.block),invwstr.block,ldif.block,wstr.block,sst.dif,fun=function(v,w,x,y,z) ifelse(is.na(z),v, w*x+(y*z)+(x*z)) )
#latent.effect<-evapdif+condendif  # latent heat effect

# Plot effect rasters
#par(mfrow=c(2,2))
#plot(rad.effect,main=paste("Rad effect ",timelabel,sep=""))
#plot(coast.effect,main=paste("Coast effect ",timelabel,sep=""))
#plot(latent.effect,main=paste("Latent effect ",timelabel,sep=""))
#plot(elev.effect,main=paste("Elev effect ",timelabel,sep=""))


# If first hour of new day set prevtref.block to last hr of previous day
#if (hr>0){prev.tref5km.r<-raster(t5km.day[,,hr],template=grid5km.r)} 
#else prev.tref5km.r<-raster(t5km.prevday[,,23],template=grid5km.r) 

# Calculate net radiation for block - from incoming shortwave, albedo and outgoing longwave radiation - NOT REQUIRED
# netr.block<-(total.block*(1-albedo))-lwr.block

# OPTION: Ref wind strength = mean for 5km block
wstr.anom.block<-wstr.block-cellStats(wstr.block, stat='mean', na.rm=TRUE)
wstr.mean<-matrix(cellStats(wstr.block, stat='mean', na.rm=TRUE),nrow=nrow(wstr.block),ncol=ncol(wstr.block) )
wstr.mean.block<-raster(wstr.mean,template=wstr.block)
wstr.mean.block<-mask(wstr.mean.block,wstr.block)

# OPTION: Ref netr = mean for 5km block
netr.mean<-matrix(cellStats(netr.block, stat='mean', na.rm=TRUE),nrow=nrow(netr.block),ncol=ncol(netr.block) )
netr.mean.block<-raster(netr.mean,template=netr.block)
netr.mean.block<-mask(netr.mean.block,netr.block)   
...
radiation_downscale<-function(day,month,year,hr,demuk,dem.buffer,dem.block,slope.buffer,aspect.buffer,dir_rad,print.results=FALSE,write.files=FALSE)
{
  ...
  if (cellStats(crop(sis.rprj,dem.block),max)>0) #  DAYTIME if no value in block >0
  {
    # resample to 100m
