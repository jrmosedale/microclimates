# This function calculates crop reference evapotranspiration (using the Penman-Monteith equation).
  # Inputs can either be a single value, in which case a single value is output, or an array of values in which
  # case the output is an array with identical dimensions
...
rh.change.v1<-function(t1,t2,rh){
...
# Input variables:
  ...
  # rh is the reference relative humdity, expressed as a percentage - i.e. the value for each 5 km grid cell
# Output variable:
 # the amount of water condensed [mm m-2 hr-1]. Zero if relative humidity is less than 100%
Water.conden.v1<-function(t1,t2,rh){
     ...
     a.him.2<-a.hum.2-a.hum.100 # CORRECTED FROM ORIGINAL
...
CRE.5km<-CRE.v1(Temp.5km,Rn.5km,RH.5km,P,dn,u2.5km)
CRE.100m<-CRE.v1(Temp.100m,Rn.100m,RH.100m,P,dn,u2.100m)
evapdif<-CRE.100m-CRE.5km # regress this against temperature in model

# Calculate difference in water condensation
wc.5km<-Water.conden.v1(Temp.5km.prev,Temp.5km,RH.5km)
wc.100m<-Water.conden.v1((Temp.5km.prev+anom),Temp.100m,RH.5km)
