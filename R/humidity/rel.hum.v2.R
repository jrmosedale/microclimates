...
rh.hourly<-function(start.jd,end.jd,dir_rh,dir_rh5km,dir_hrtemp, grid5km.r,hourplot=FALSE)
  ...
  for (jd in start.jd:end.jd)
  {
    ...
    filein1<-paste(dir_hrtemp,"HrTemp_", DMYjd(jd)$year[1], "-",sprintf("%02d",DMYjd(jd)$month[1],sep=""),"-", sprintf("%02d",DMYjd(jd)$day[1],sep=""),".r", sep="") # define file name from year,month,day,hr
    print(filein1)
    load(filein1)
    hr.temp1<-t5km.day
    
    filein2<-paste(dir_hrtemp,"HrTemp_", DMYjd(jd+1)$year[1], "-",sprintf("%02d",DMYjd(jd+1)$month[1],sep=""),"-", sprintf("%02d",DMYjd(jd+1)$day[1],sep=""),".r", sep="") # define file name from year,month,day,hr
    print(filein2)
    load(filein2)
    hr.temp2<-t5km.day
