...
start.jd<-JDdmy(1,4,start.year)
end.jd<-JDdmy(30,11,end.year)
print(start.jd)
print(end.jd)
#jd<-start.jd
...
year<-2012
year<-2010
stat<-paste(dir_gdd,"GDD10hr-aproct-",sep="")
stat<-paste(dir_frost,"SpringTmin-aprmay-",sep="")
stat<-paste(dir_flowering,"Flowering-tmean-",sep="")
stat<-paste(dir_flowering,"Flowering-numday-",sep="")

par(mfrow=c(1,1))
stats<-c(paste(dir_gdd,"GDD10hr-aproct-",sep=""),paste(dir_frost,"SpringTmin-aprmay-",sep=""),paste(dir_flowering,"Flowering-tmean-",sep=""),paste(dir_flowering,"Flowering-numday-",sep=""))
titles<-c("GDD Apr-Oct","Min Temperature Apr-May","Mean Flowering Temperature", "Flowering days >15C" )
...
#plot.new()
plot(map.r, col=rev(terrain.colors(length(seq(500, 1200, by = 50))-1)),axes=FALSE,box=FALSE, breaks=seq(500, 1200, by = 50),legend.only=T,)
# heat colour scheme
breaks<-seq(500,1500,by=10)
plot(map.r, col=rev(heat.colors(50)),axes=FALSE,box=FALSE)
#plot(map.r, col=(topo.colors(14,start=500,end=1200)),axes=FALSE,box=FALSE)

# FOR fl days
brks <- seq(0, 14, by=1) 
plot(map.r, col=rev(heat.colors(14)),axes=FALSE,box=FALSE)

plot(map.r, col=rev(heat.colors(length(seq(0, 14, by = 1))-1)),xaxs="i",yaxs="i",axes=FALSE,box=FALSE, breaks= seq(0, 14, by = 1), legend=T)
plot.new()
plot(map.r, col=rev(heat.colors(length(seq(0, 14, by = 1))-1)),axes=FALSE,box=FALSE, breaks= seq(0, 14, by = 1), legend.only=T)


#plot(map.r,main=paste(titles[x],year))
}
