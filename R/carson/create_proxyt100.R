...
dir_temp<-paste(in.root,"Temp5km/unzip/",sep="")
...
hcounties.shp<- readOGR(dsn = path.expand(paste(root,"OSdata/",sep="")), layer = "Boundary-line-historic-counties_region")
...
##########################################################################################  
  for (jd in JDdmy(1,1,year) :JDdmy(31,12,year)){
    ...
    # effect of cold air drainage  
      final.tmin<-day.tmin
      if ((DMYjd(jd)$month<4 | DMYjd(jd)$month>10) & cellStats(day.tmin,min)<3) {
       inversion<-runif(1)/3
