...
#start.year<-1992; start.month<-7; start.day<-10; hr<-0
#end.year<-1992; end.month<-7; end.day<-15
...
### Load PARAMETERS ###
parameters<-"/home/ISAD/jm622/rscripts/inputs/parameters.R")
...
##########################################################################################
# Progs:  Prepare data required for ANY cell
##########################################################################################
...
##########################################################################################
# Progs:  Prepare data required for UKCP cell fixed for all timeperiods
##########################################################################################
...
for (jd in start.jd:end.jd ) 
  {
    ...
    for (hr in 0:23)  #loop for hours
    {
      ...
      ### CALC COASTAL EFFECT ###
      print("Coastal")
      # Calculate upwind.sst & sst-tref for block - record NA if no sea cell upwind within 20km
