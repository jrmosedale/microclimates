...
# v2 - loads most data at daily NOT hourly (v1) step 
##########################################################################################
...
source("/home/ISAD/jm622/rscripts/apply2_model_functions.R") # loads & runs functions
print("Calling inputs...")
source("/home/ISAD/jm622/rscripts/apply2_model_inputs.R") # defines inputs,parameters, etc
...
# start.day <- 1; start.month<-7 ; start.year<- 1992; hr<-0
# end.day<-31; end.month<-7; end.year<-1992
# ukcpcell<-936 
##########################################################################################
...
for (jd in start.jd:end.jd ) 
  {
    ...
    #prevtref.block<-tps.resample(t5km.prevhr,dem.block) # CREATES Tref SURFACE
    ...
    for (hr in 0:23)  #loop for hours
    {
      ...
      #tref.block<-tps.resample(tref5km.r,dem.block) # CREATES Tref SURFACE
      ...
      wind.results<-wind.tpsdownscale(day,month,year,hr,dem.block,wind_u,wind_v,dir_wind,dir_shelter,shelter.block,interval=10,print.res=FALSE,write.files=FALSE )
      wdir.block<-wind.results[[1]]
      wstr.block<-wind.results[[2]]
      invwstr.block<-wind.results[[3]]     
      #invwstr.dif<-invwstr.ref-invwstr.block
      
      print("Start Rad processes ") ; print(proc.time()[1:3] - ptm[1:3])
      ...
      print("Start LWR processes ") ; print(proc.time()[1:3] - ptm[1:3])
      ...
      print("End all Rad processes ") ; print(proc.time()[1:3] - ptm[1:3])
      ...
      t.block<-tps.resample(tref5km.r,dem.block)+anom.block # CREATES Tref SURFACE+anom.block after interpolating t5km.r
      tanom.block<-t.block-tref.block # Check correct order/sign - used in next hour timestep
      
      print("Completed all model processes ") ; print(proc.time()[1:3] - ptm[1:3])
      ...
      print("Completed plotting ") ; print(proc.time()[1:3] - ptm[1:3])
    ...
    # load by: test<-stack(paste(dir_finalt,"name.tif",sep=""))
      ...
      print("Written day file ") ; print(proc.time()[1:3] - ptm[1:3])
