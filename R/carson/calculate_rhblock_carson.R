...
###########################################################################
rh.hourly<-function(jd,dir_rh,dir_rh5km,dir_hrtemp,dem,dem.block,hourplot=FALSE,writefile=FALSE)
{
  ...
  #e<-extent(353.75,359.25,48.75,53.75)
    ...
    { print(paste("Period= ",period,sep=""))
      ...
      # Convert absolute humidity to 100m grid cell resolution 
      abs.h0<-reproject.r(abs.hum0,e,dem.block)
      abs.h1<-reproject.r(abs.hum1,e,dem.block)
      abs.h2<-reproject.r(abs.hum2,e,dem.block)
      abs.h3<-reproject.r(abs.hum3,e,dem.block)
      abs.h4<-reproject.r(abs.hum4,e,dem.block)
      abs.h5<-reproject.r(abs.hum5,e,dem.block)
      abs.h6<-reproject.r(abs.hum6,e,dem.block)
      
      # Load remaining hrs of 100m temperature data and crop all to dem.block
      t0.100m<-crop(t0.100m,dem.block)
      t1.100m<-crop(raster(hr.temp1[,,hr+2],template=dem),dem.block)
      t2.100m<-crop(raster(hr.temp1[,,hr+3],template=dem),dem.block)
      t3.100m<-crop(raster(hr.temp1[,,hr+4],template=dem),dem.block)
      t4.100m<-crop(raster(hr.temp1[,,hr+5],template=dem),dem.block)
      t5.100m<-crop(raster(hr.temp1[,,hr+6],template=dem),dem.block)
      t6.100m<-crop(t6.100m,dem.block)
      
      # Convert to 100m relative humidity
      hr<-period*6
      rh.day[,,hr+1]<-abs.to.rel(getValues(abs.h0,format="matrix"),getValues(t1.100m,format="matrix"))
      rh.day[,,hr+2]<-abs.to.rel(getValues(abs.h1,format="matrix"),getValues(t2.100m,format="matrix"))
      rh.day[,,hr+3]<-abs.to.rel(getValues(abs.h2,format="matrix"),getValues(t3.100m,format="matrix"))
      rh.day[,,hr+4]<-abs.to.rel(getValues(abs.h3,format="matrix"),getValues(t4.100m,format="matrix"))
      rh.day[,,hr+5]<-abs.to.rel(getValues(abs.h4,format="matrix"),getValues(t5.100m,format="matrix"))
      rh.day[,,hr+6]<-abs.to.rel(getValues(abs.h5,format="matrix"),getValues(t6.100m,format="matrix"))
    ...
    # WRITE DAILY files of 100m RELATIVE humidity
    if (writefile==TRUE){
...
#######################################################################################
# FUNCTION CALL
rh.hourly(start.jd,end.jd,dir_rh,dir_rh5km,dir_hrtemp,dem,dem.block)
