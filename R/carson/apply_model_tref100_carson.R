...
for (jd in start.jd:end.jd ) 
  {
    ...
    fileout<-paste(dir_finalt,"modelt-",ukcpcell,"-", DMYjd(jd)$year, "-",sprintf("%02d",DMYjd(jd)$month,sep=""),"-", sprintf("%02d",DMYjd(jd)$day,sep=""),sep="")
    print(fileout)
    writeRaster(tmodel.day,file=paste(dir_finalt,"block-",ukcpcell,"-",day,"-",month,"-",year,".tif",sep=""), format="GTiff",overwrite=TRUE)
