...
#start.doy<-1 
start.doy<-daydoy(year,month,day)-1
...
for (hr in 10:14){
  ...
  for (cellnum in cells){
    ...
    infile<-paste(dir_finalt,"block20km-",sprintf("%03d",cellnum,sep=""),"-",year,sep="")
    print(infile)
    load(infile) 
    numdays<-dim(results)[3]/24
    print(numdays)
...
writeRaster(map.24h,file=paste(dir_tmaps,"tmap24h_4cells_",datelabel,".tif",sep=""))
