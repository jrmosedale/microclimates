...
#end.day<-4; end.month<-4; end.year<-1992
ptm <- proc.time()
...
##########################################################################################
  ...
  tmodel.r<-array(NA,c(200,200,max.hr)) #### CHANGE HERE IF DIFFERENT BLOCK SIZES USED #####
  ...
  fileout<-paste(dir_finalt,"block20km-",sprintf("%03d",ukcpcell,sep=""),"-",year,".R",sep="")
