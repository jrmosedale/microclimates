...
# New Version - uses inv.dist maps calculated using circle sector not line
# Terrain functions that convert sea to height 0 not NA
# 
##########################################################################################
...
#####################################################################
# TERRAIN  MAPS
# Requires converting NA to 0 elevation
#####################################################################

calcslope_deg<-function(dem){
  r<-calc(dem,function(x){ifelse(is.na(x),0,x)})
  slope.r<-terrain(r,"slope", unit='degrees', neighbors=8)
  slope.r<-mask(slope.r,dem)
  return(slope.r)
}

calcaspect_deg<-function(dem){
  r<-calc(dem,function(x){ifelse(is.na(x),0,x)})
  aspect.r<-terrain(r,"aspect", unit='degrees', neighbors=8)
...
#####################################################################
interpolated.elevation.dif.map<-function(dem,grid5km.r,dir_terrain){
  #refgrid.r<-crop(uk5kmgrid.r,dem)
  if (extent(dem)!=extent(grid5km.r)) Warning("Input rasters for calculating interpolated elevation DO NOT have same extents")
  
  # A. Calculate reference height for Met Off 5km gridcells from 100m cells matching central xy
  # Extract from central xy and set to 0 if NA (sea/water)
  xy<-xyFromCell(grid5km.r,1:ncell(grid5km.r))
  elev<-extract(dem, xy) 
  elev<-ifelse(is.na(elev),0,elev)
  elev5km.r<-setValues(grid5km.r,elev)
  elev5km.r<-mask(elev5km.r,grid5km.r)
  plot(elev5km.r,main="Elev mean 5km")
  # Or use 5km mean elevation
  #elev5km.r<-mask(aggregate(dem,fact=c(50,50),fun=mean),refgrid.r) 
...
wind_sheltermaps<-function(demuk,dem,interval,dir_shelter){
  ...
  buffer<-10000
  e.map<-extent(xmin(dem)-buffer,xmax(dem)+buffer,ymin(dem)-buffer,ymax(dem)+buffer)
...
percent_land_maps<-function(dembuf,grid5km.r,dem,radius,dir_percland) {
  ...
  #  Set sea to NA and output 100m version - OUTPUT USED IN CALC OF LDIF MAPS
...
# Terrain maps - used by radprog_blocks
 slope.r<-calcslope_deg(dem)
 aspect.r<-calcaspect_deg(dem)
 plot(aspect.r,main="Aspect")
 plot(slope.r,main="Slope")
 writeRaster(slope.r,file=paste(dir_terrain,"slope.tif",sep=""),overwrite=TRUE)
 writeRaster(aspect.r,file=paste(dir_terrain,"aspect.tif",sep=""),overwrite=TRUE)
...
wind_sheltermaps(demuk,dem,interval,dir_shelter)
