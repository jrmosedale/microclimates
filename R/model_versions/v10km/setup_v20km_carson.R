...
#land5km.file<-paste(dir_dem,"land5km.tif",sep="") # 1 or 0 according to whether any 100m land cells 
...
#land5km.r<-raster(land5km.file)  # used in downscaling pressure (to 5km is adequate)
demland.r<-raster(demland.file)

# Calculate 100m land cells in each 20x20km cell
# NOTE 1 Keeps cells within original dem extent 
# NOTE 2: flips then re-flips raster eo ensure same bottom left (SW) origin - trips on N & E axes
land100m.r<-calc(dem,function(x) ifelse(is.na(x),NA,1))
land100m.r<-flip(land100m.r,direction='y')
land20km.r<-aggregate(land100m.r,fact=200,fun=sum,expand=FALSE) # = number of 100m land cells in each 5km cell
land100m.r<-flip(land100m.r,direction='y')
land20km.r<-flip(land20km.r,direction='y')
land100m.r<-flip(land100m.r,direction='y')
land20km.r<-calc(land20km.r,function(x) ifelse(is.na(x),NA,1)) # convert to either 1 or NA 
extent(land20km.r)<-extent(c(120000,420000,10000,170000)) # re-estalish correct extent after flip 
#print(paste("Number of 20x20km cells with 1+ 100m land cells = ", length(which(getValues(land20km.r)==1))) ) 

# CREATE Reference dataset for defining cells used in analysis - 20km and 10km cells
# Create matrix of min max xy values for each 20km or 10 km grid cell used in processing
vals<-getValues(land20km.r)
xy<-xyFromCell(land20km.r,1:ncell(land20km.r))
sel<-which(vals==1)
landcells20km<-xy[sel,1:2]
cells20km.df<-as.data.frame(landcells20km)   # = coordinates for middle of each ukcp09 cell
cells20km.df$xmin<-cells20km.df$x-10000
cells20km.df$xmax<-cells20km.df$x+10000
cells20km.df$ymin<-cells20km.df$y-10000
cells20km.df$ymax<-cells20km.df$y+10000
#print(dim(cells20km.df))

# Add N row of 10x10km cells 
toprow.r<-crop(dem,extent(120000,420000,170000,180000))
toprow100m.r<-calc(toprow.r,function(x) ifelse(is.na(x),NA,1))
toprow10km.r<-aggregate(toprow100m.r,fact=100,fun=sum,expand=FALSE)
toprow10km.r<-calc(toprow10km.r,function(x) ifelse(is.na(x),NA,1)) # convert to either 1 or NA 
#print(paste("Number of 20x20km cells with 1+ 100m land cells = ", length(which(getValues(toprow10km.r)==1))) ) 
vals<-getValues(toprow10km.r)
xy<-xyFromCell(toprow10km.r,1:ncell(toprow10km.r))
sel<-which(vals==1)
landcells10km<-xy[sel,1:2]
cells10km.df<-as.data.frame(landcells10km)
cells10km.df$xmin<-cells10km.df$x-5000
cells10km.df$xmax<-cells10km.df$x+5000
cells10km.df$ymin<-cells10km.df$y-5000
cells10km.df$ymax<-cells10km.df$y+5000
#print(dim(cells10km.df))

modelcells<-rbind(cells10km.df,cells20km.df)
