...
#
# Version 2 - interpolates %land maps to avoid cell blocking and account for interpolation of tref data
##########################################################################################
...
percent_land_maps<-function(dembuf,grid5km.r,dem,radius,dir_percland) {
  ...
  #coast100m.r<-mask(coast.r,crop(dembuf,dem))
  ...
  # B Interpolate 5km &land using tps back to 100m cells for whole dem
  pland100m.r<-tps.resample(plandref5km.r,dem,maskoutput=TRUE) 
  plot(pland100m.r,main=" 5lm %land interpolated to 100m")
  
 # out.file<-paste(dir_percland,"percent_land_",radius/1000,"km_lref_100mgrid.tif",sep="")
 # writeRaster(mask(coast.r,dem),file=out.file,overwrite=T)
  
  out.file<-paste(dir_percland,"percent_land_",radius/1000,"km_lref_100mgrid.tif",sep="")
  writeRaster(pland100m.r,file=out.file,overwrite=T)
...
ldif.maps<-function(interval,radius,dir_percland,dir_lsratio,dir_ldif){
  ...
  for (direction in seq(0,360,interval))
  {
    ...
    projection(inv.lsratio)<-CRS("+init=epsg:27700")
    inv.lsratio<- crop(inv.lsratio,dem)
    compareRaster(inv.lsratio,pland100m.r)
