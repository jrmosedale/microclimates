...
#source("/home/ISAD/jm622/rscripts/apply_model_v1_functions.R") # loads & runs functions
print("Calling inputs...")
#source("/home/ISAD/jm622/rscripts/apply_model_v1_inputs.R") # defines inputs,parameters, etc
...
source("~/Documents/Exeter/RProjects/Project2015/model_versions/v1/apply_model_v1_inputs.R")
...
for (jd in start.jd:end.jd) 
  {
    ...
    tmodel.day<-stack()
    radef.day<-stack(); latef.day<-stack();windstr.day<-stack();tanom.day<-stack()
    ...
    for (hr in 0:23)  #loop for hours
    {
      ...
      radef.day<-stack(radef.day,RadEffect); latef.day<-stack(latef.day,LatentEffect);windstr.day<-stack(windstr.day,wstr.block);tanom.day<-stack(tanom.day,tanom.block)
    ...
    fileout<-paste(dir_finalt,"wind-",ukcpcell,"-",sprintf("%02d",DMYjd(jd)$day,sep=""),"-",sprintf("%02d",DMYjd(jd)$month,sep=""),"-",year,".tif",sep="")
    writeRaster(windstr.day,fileout,format="GTiff",overwrite=TRUE)
    fileout<-paste(dir_finalt,"tanom-",ukcpcell,"-",sprintf("%02d",DMYjd(jd)$day,sep=""),"-",sprintf("%02d",DMYjd(jd)$month,sep=""),"-",year,".tif",sep="")
    writeRaster(tanom.day,fileout,format="GTiff",overwrite=TRUE)
