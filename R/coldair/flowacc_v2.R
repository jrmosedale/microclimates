...
## Correct and re-write outputs
flow.r<-raster(paste(dir.basinmap,"flowacc-all.tif",sep=""))
projection(flow.r)<-"+init=epsg:27700"
flow.r<-extend(flow.r,e)
flow.r<-shift(flow.r,-50,-50) 
writeRaster(flow.r,file=paste(dir.basinmap,"flowacc-all.tif",sep=""),overwrite=TRUE)

topidx.r<-raster(paste(dir.basinmap,"topidx.tif",sep=""))
projection(topidx.r)<-"+init=epsg:27700"
topidx.r<-extend(topidx.r,e)
topidx.r<-shift(topidx.r,-50,-50) 
writeRaster(topidx.r,file=paste(dir.basinmap,"topidx.tif",sep=""),overwrite=TRUE)

altrange.r<-raster(paste(dir.basinmap,"altrange.tif",sep=""))
projection(altrange.r)<-"+init=epsg:27700"
altrange.r<-extend(altrange.r,e)
altrange.r<-shift(altrange.r,-50,-50) 
writeRaster(altrange.r,file=paste(dir.basinmap,"altrange.tif",sep=""),overwrite=TRUE)

altdif.r<-raster(paste(dir.basinmap,"altdif.tif",sep=""))
projection(altdif.r)<-"+init=epsg:27700"
altdif.r<-extend(altdif.r,e)
altdif.r<-shift(altdif.r,-50,-50) 
writeRaster(altdif.r,file=paste(dir.basinmap,"altdif.tif",sep=""),overwrite=TRUE)
